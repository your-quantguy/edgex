...
class Manager:
    ...
    def subscribe_trade_event(self, handler: Callable[[str], None]):
        """
        Subscribe to all trade events (including order updates, account updates, etc.).
        ...
        client.on_message("trade-event", handler)

    def subscribe_specific_trade_event(self, event_type: str, handler: Callable[[str], None]):
        """
        Subscribe to a specific trade event type.

        Args:
            event_type: The event type (e.g., "ORDER_UPDATE", "ACCOUNT_UPDATE")
            handler: The handler function
        ...
        client.on_message(event_type.lower(), handler)
