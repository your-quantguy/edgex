    ...
    def _handle_messages(self):
        ...
        while not self.done.is_set():
            ...
            try:
                ...
                # Handle private WebSocket messages (trade-event type)
                if self.is_private and msg.get("type") == "trade-event":
                    content = msg.get("content", {})
                    event = content.get("event", "")
                    
                    # Map events to handlers
                    if event == "ORDER_UPDATE" and "order" in self.handlers:
                        self.handlers["order"](message)
                    elif event == "ACCOUNT_UPDATE" and "account" in self.handlers:
                        self.handlers["account"](message)
                    elif event == "POSITION_UPDATE" and "position" in self.handlers:
                        self.handlers["position"](message)
                    elif event == "DEPOSIT_UPDATE" and "deposit" in self.handlers:
                        self.handlers["deposit"](message)
                    elif event == "WITHDRAW_UPDATE" and "withdraw" in self.handlers:
                        self.handlers["withdraw"](message)
                    elif event == "TRANSFER_IN_UPDATE" and "transfer_in" in self.handlers:
                        self.handlers["transfer_in"](message)
                    elif event == "TRANSFER_OUT_UPDATE" and "transfer_out" in self.handlers:
                        self.handlers["transfer_out"](message)
                    elif event == "FORCE_WITHDRAW_UPDATE" and "force_withdraw" in self.handlers:
                        self.handlers["force_withdraw"](message)
                    elif event == "FORCE_TRADE_UPDATE" and "force_trade" in self.handlers:
                        self.handlers["force_trade"](message)
                    elif event == "FUNDING_SETTLEMENT" and "funding_settlement" in self.handlers:
                        self.handlers["funding_settlement"](message)
                    elif event == "ORDER_FILL_FEE_INCOME" and "order_fill_fee" in self.handlers:
                        self.handlers["order_fill_fee"](message)
                    elif event == "START_LIQUIDATING" and "start_liquidating" in self.handlers:
                        self.handlers["start_liquidating"](message)
                    elif event == "FINISH_LIQUIDATING" and "finish_liquidating" in self.handlers:
                        self.handlers["finish_liquidating"](message)
                    elif event == "UNRECOGNIZED" and "unrecognized" in self.handlers:
                        self.handlers["unrecognized"](message)
                    
                    # Also call the generic trade-event handler if registered
                    if "trade-event" in self.handlers:
                        self.handlers["trade-event"](message)
                    
                    continue

                # Handle quote events (public WebSocket)
